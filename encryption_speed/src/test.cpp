...
// Add CPU cycle counting functions
#ifdef __x86_64__
...
void test_ZZ_p_k(int k)
{
    ...
    // Create message sequence of size k * 16 (since each element represents 128 bits)
    int message_length = k;
    Vec<ZZ_p> Mseq;
    Mseq.SetLength(message_length);
    
    // Fill with test data (alternating pattern for demonstration)
...
#ifdef __x86_64__
    // CPU cycle measurement (more accurate)
    uint64_t start_cycles = rdtscp();  // Serializing version
    tie(Cseq, tag) = authenc(MK, nonce, Mseq);
    uint64_t end_cycles = rdtscp();
    uint64_t cycles = end_cycles - start_cycles;
#else
    tie(Cseq, tag) = authenc(MK, nonce, Mseq);
    uint64_t cycles = 0; // Not available on non-x86 platforms
#endif
    ...
    cout << "Encryption of " << k << " * 128 bits (" << k * 128 << " total bits) completed\n";
    cout << "Message length: " << message_length << " elements\n";
    cout << "Encryption time: " << duration.count() << " microseconds\n";
    cout << "Encryption time: " << duration.count() / 1000.0 << " milliseconds\n";
    
#ifdef __x86_64__
    cout << "CPU cycles: " << cycles << " cycles\n";
    if (duration.count() > 0) {
        double cycles_per_us = (double)cycles / duration.count();
        cout << "CPU cycles per microsecond: " << cycles_per_us << "\n";
        cout << "Cycles per bit: " << (double)cycles / (k * 128) << "\n";
    }
#endif
}
...
int main()
{
	//test_ZZ_p();
	//test_ZZ_pE();
 // Test different sizes: 1*128, 2*128, 4*128, 8*128, 16*128 bits
 vector<int> test_sizes = {132, 64, 128, 256, 512, 1024, 2048, 4096};
    
 for (int k : test_sizes) {
	 cout << "\n========== Testing k=" << k << " (encrypting " << k * 128 << " bits) ==========\n";
	 test_ZZ_p_k(k);
 }
