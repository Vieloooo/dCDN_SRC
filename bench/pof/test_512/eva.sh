...
function setup() {
    pushd "$SCRIPT_DIR"
    "${TIME[@]}" snarkjs groth16 setup test_pof.r1cs ${TAU_FILE} tk0.zkey
    echo entro | snarkjs zkey contribute tk0.zkey tk1.zkey --name='Vielo' -v
    "${TIME[@]}" snarkjs zkey export verificationkey tk1.zkey verification_key.json
...
function generateWtns() {
    ...
    avg_time 3 node test_pof_js/generate_witness.js test_pof_js/test_pof.wasm input.json witness.wtns
    popd
}
